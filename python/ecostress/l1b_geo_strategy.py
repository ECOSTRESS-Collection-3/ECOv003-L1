...
class L1bGeoStrategy2Pass(L1bGeoStrategy):
    ...
    def modify_igc(
        ...
        orb = igccol.orbit
        if pass_number == 1:
            for i in igccol.number_image:
                if len([tp for tp in tpcol if tp.image_coordinate(i) is not None]) > 1:
            ...
            for i in igccol.number_image:
                if (
                    len([tp for tp in tpcol if tp.image_coordinate(i) is not None]) > 1
                ) and l1b_geo_process.scene_list[i] not in orb.scene_list:
                    tmin, _, tmax = self.image_time(igccol, i)
    ...
    def correct_igc(
        ...
        self.collect_qa(
            l1b_geo_process, igccol_corrected_pass1, tpcol_pass1, pass_number=1
        )
        igccol_corrected, tpcol = self.correct_igc_pass(
            l1b_geo_process, igccol_corrected_pass1, pool, pass_number=2
        )
        self.collect_qa(l1b_geo_process, igccol_corrected, tpcol, pass_number=2)
