...
class L1aRawPixGenerate(object):
    ...
    def process_scene_file(self) -> list[tuple[int, int, Time, Time]]:
        ...
        with open(self.scene_file, "r") as fh:
            for ln in fh:
                orbit, scene_id, sc_start_time, sc_end_time = re.split(
                    r"\s+", ln.strip()
                )
                orbit = int(orbit)
    ...
    def detect_obst(self, sts: Time, ste: Time) -> str:
        ...
        for file_name in glob.glob(obst_files):
            ...
            if ste < t1:
                break  #  time codes beyond scene end
            ...
            for i, evl in enumerate(ef):
                ...
                if i < 4:
                    ...
                    if int(e4) < int(e3):  # compensate for zero-crossing in EV range
                        ...
                        ev_codes[i, 3] = int(e4)
                    print(
                        "P1=%d P2=%d P3=%d P4=%d" % (int(e1), int(e2), int(e3), int(e4))
                    )
                    print(
                        "EV_CODES[%d](%s) = (%d,%d) (%d,%d) (%d,%d)"
                        % (
                            i,
                            ev_names[i],
                            ev_codes[i, 0],
                    ...
                    FPPSC = int(e4)
                    print(
                        "RPM=%f FP_DUR=%20.10f MAX_FPIE=%d FPPSC=%d"
    ...
    """
        ...
        if m:
            onum = m.group(1)
            print(
                "Orbit number from file name: %s USE_OBST_FILE=%s"
                % (onum, self.use_obst_file)
            )
        else:
            print(
                "*** Error:  Could not find orbit number from L0B file name %s"
                % self.l0b
            )
            return -1
        ...
        if epc > 0:
            eng, eng_met, fname = self.create_file(
                "L1A_ENG",
                int(onum),
                None,
        ...
        if aqc > 0:
            attf, attf_met, attfname = self.create_file(
                "L1A_RAW_ATT",
                int(onum),
                None,
      ...
      """
        ...
        gpt = self.l0_time_calc.gps_time_for_orbit(
            fswt, fsw_sync, fpie_sync, self.process_scene_file(), int(onum)
        )
        ...
        for orbit, scene_id, sts, ste in self.process_scene_file():
            ...
            if orb != onum:  # process only matching orbit numbers
                print(
                    "Ignoring mismatch orbit number %s, ref=%s scene=%d"
                    % (orb, onum, scene_id)
                )
                continue
            print("====  ", datetime.now(), "  ====")
            dt = ste.gps - sts.gps
            print(
                "SCENE=%03d START=%s(%f) END=%s(%f) DT=%f"
                % (scene_id, sts, sts.gps, ste, ste.gps, dt)
            )
            ...
            if fov_obst == "NA":
                print(
                    "Error:  Obstruction files not found in DIR %s, terminating"
                    % self.obst_dir
                )
            ...
            dt = t0 - sts
            print(
                "Located scene %s start time %f in PKT[%d] %s(%f) DT=%f"
                % (scene_id, sts.gps, pkt_idx, str(t0), t0.gps, dt)
            )
            rst = t0.gps - dt
            rse = ste.gps  # initialize refined scene end time
            print(
                "Initial guess of refined scene start/end time %s(%f) %s(%f)"
                % (Time.time_gps(rst), rst, Time.time_gps(rse), rse)
            )
            ...
            scans = 0
            if pkt_idx > 0:
                pkt_idx -= 1  # compensate for time code error
                ...
                while seq < 3:
                    ...
                    while e0 < tot_pkts and ph == 2:  # search for sequence start
                        ...
                        if gpt[e0] > rse:  # packet time past end of scene
                            ...
                            print(
                                "** Finish orbit %05d short scene %s SCANS=%d end=%s(%f) GPT=%s(%f) at %d"
                                % (
                                    orbit,
                                    scene_id,
                                    scans,
                        ...
                        if adt > SCAN_DUR and seq > 0:
                            print(
                                "** Discontinuity seeking %s IDX=%d DT=%f E1=%d"
                                % (ev_names[seq], e0, dt, e1)
                            )
                            cont = 0
                        ...
                        while e1 < FPPPKT and ph == 2:
                            ...
                            if lid0 >= ev_codes[seq, 0] and lid0 <= ev_codes[seq, 1]:
                                ph = 0
                            elif lid1 >= ev_codes[seq, 2] and lid1 <= ev_codes[seq, 3]:
                                ph = 1
                            else:
                                e1 += 1  # check next EV
                    ...
                    pkt_idx = e0
                    if cont == 0:
                        break  # discont in SEQ seek, break out of SEQ loop
          ...
          """
                    ...
                    if adt > PKT_DURT or e0 >= tot_pkts - 1:
                        print(
                            "Scene %d Disco %s, terminating scan %d E0=%d E1=%d DT=%f"
                            % (scene_id, ev_names[seq], scan, e0, e1, dt)
                        )
                    ...
                    if seq == 0:  # save scan start time
                        ...
                        sst0 = sst
                        print(
                            "Calculated scan=%02d SCENE=%s SST=%f RST=%f DST=%f STD=%9f FPD=%9f"
                            % (scan, scene_id, sst, rst, dst, std, fpd)
                        )
                        if scan >= SCPS:
                            print(
                                "PKT[%d] Time %f outside of current scene %d Terminating"
                                % (e0, gpt[e0], scene_id)
                            )
                            cont = 0
                            break
                    ...
                    while op < op1 and e0 < tot_pkts:
                        ...
                        if e0 == tot_pkts - 1:  # at last packet in file
                            ...
                            if fswtc_err == 1:
                                lid0 = e0 - 1
                                lid1 = e0  # time code error
                            else:
                                lid0 = e0
                                lid1 = e0 + 1  # correct time
                        ...
                        if (
                            op > op0
                            and (dt < 0 or abs(dt - PKT_DUR) > PKT_DURT)
                            and seq == 2
                        ):  # time discontinuity
                            ldd[:] = (
                            ...
                            if fpc < remain:  # jump occurred before end of IMG
                                jumps += 1
                                print(
                                    "*** Orbit %s Scene %d scan %d %s Time jump PKT=%d %s DT=%10.8f OP=%d "
                                    % (
                                        orb,
                                        scene_id,
                                        scan,
                                ...
                                if (
                                    dt > IMG_DUR - (op1 - op) * FP_DUR
                                ):  # jump outside of current IMG, go to next scan
                                ...
                                if e0 == tot_pkts - 1:  # at end of data
                                    ...
                                    dev = abs(
                                        (lev[lid1, 0] - lev[lid0, 0]) - FP_EV * FPPPKT
                                    )
                                    d2 = abs(
                                        (lev[lid1, FPPPKT - 1] - lev[lid0, FPPPKT - 1])
                                        - FP_EV * FPPPKT
                                    )
                                if (
                                    (op == op0 and d2 <= FP_EVT)
                                    or (op >= op1 - FPPPKT and dev <= FP_EVT)
                                    or (dev <= FP_EVT and d2 <= FP_EVT)
                                ):
                                    opinc = 0  # EV continuous
                                else:
                                    opinc = int(dt / FP_DUR + 0.5) - FPPPKT

                                if dt < 0:
                                    print("Negative time jump", end="")
                                else:
                                    print(" ...continuing", end="")

                                print(" OPINC=%d  ***" % opinc)
                                print(
                                    "Copying remaining %d FPs from PKT [%d,%d] to scan %d at %d"
                                    % (fpc, e0, p1, scan, op)
                                )
                            # End jump detection
                        ...
                        if fpc >= remain:
                            ...
                            if seq == 2:
                                ...
                                print(" NULL")
                            if remain == FPPPKT:
                                remain = 0  # next search in next packet

                        dp = op - op0
                        # print("SEQ=%d LINE=%d OP=%d DP=%d P1=%d FPC=%d E0=%d REMAIN=%d OPINC=%d" %(seq,line,op,dp,p1,fpc,e0,remain,opinc))
                        obuf[seq][line : line + PPFP, dp : dp + fpc, :] = flex_buf[
                            :, p1 : p1 + fpc, :
                        ]
                        if seq == 2:
                            ev_buf[scan, dp : dp + fpc] = lev[e0, p1 : p1 + fpc]
                        gpix[seq] += fpc

                        if (
                            (e0 < tot_pkts - 1)
                            and pid[lid1] > 1
                            and (int(pid[lid1]) - int(pid[lid0])) != 1
                        ):  # skip non-consecutive packet ID
                            print(
                                "found non-contiguous PKT %d PIDs=%d %d"
                                % (lid1, pid[lid0], pid[lid1])
                            )
                    ...
                    if e0 >= tot_pkts:
                        ...
                        print(
                            "Hit EOF, E0=%d, set scene end time to end of last packet: %f"
                            % (e0, sse)
                        )
                    else:
                        if op > op0 and remain > 0:
                            pkt_idx = e0 - 1
                        else:
                            pkt_idx = e0
                    if cont == 0:
                        break  # drop into scan loop
                    seq += 1
                # end seq loop

                print(
                    "SCENE=%s SCAN=%d SCANS=%d LINE=%d DT=%f s2k=%f IDX=%d REMAIN=%d OP=%d RSE=%f"
                    % (
                        scene_id,
                        scan,
                        scans,
            ...
            if o_start_time is None:
                o_start_time = Time.time_gps(rst)
            ...
            l1a_fp, l1a_fp_met, pname = self.create_file(
                "L1A_RAW_PIX",
                orbit,
                scene_id,
                Time.time_gps(rst),
            ...
            print("====  %s  ===" % sst)
            print(
                "Orbit %s SCENE %s completed, SCANS=%d (%f) %d/%d GOOD/IMG, (%f) %d/%d GOOD/BB"
                % (
                    orb,
                    scene_id,
                    scans,
            ...
            l1a_qamissing = l1a_metag.create_dataset(
                "QAPercentMissingData", data=pcomp, dtype="f4"
            )
            ...
            t = l1a_ptg.create_dataset(
                "line_start_time_j2000", data=pix_time, dtype="f8"
            )
            ...
            for b in range(BANDS):
                ...
                if e0 == 0 and img[0, 0, bo[b]] == 0xFFFF:
                    BandSpec[b] = 0.0
                else:
                    BandSpec[b] = bs[b]

                t = l1a_bpg.create_dataset(
            ...
            l1a_BandSpec = l1a_metag.create_dataset(
                "BandSpecification", data=BandSpec, dtype="f4"
            )
            ...
            l1a_qamissing = l1a_metag.create_dataset(
                "QAPercentMissingData", data=bcomp, dtype="f4"
            )
            l1a_qamissing.attrs["Units"] = "percentage"
            l1a_qamissing.attrs["valid_min"] = 0
            l1a_qamissing.attrs["valid_max"] = 100
            ...
            if p1 <= p0:
                ...
                p1 = epc - 1
            print(
                "Copying RTD for SCENE %d P0=%d P1=%d RST=%f RSE=%f %s"
                % (scene_id, p0, p1, rst, rse, str(datetime.now()))
            )
            print(" ")
            ...
            if pcomp < 50.0:
                ...
                try:
                    ...
                    if abs(mi_fp.ulc_x - mi_fp.lrc_x) < 10.0:
                        ...
                        l1a_fp["/StandardMetadata/EastBoundingCoordinate"][()] = (
                            mi_fp.lrc_x
                        )
                        l1a_fp["/StandardMetadata/SouthBoundingCoordinate"][()] = (
                            mi_fp.lrc_y
                        )
                        l1a_fp["/StandardMetadata/NorthBoundingCoordinate"][()] = (
                            mi_fp.ulc_y
                        )
                        l1a_fp["/StandardMetadata/WestBoundingCoordinate"][()] = (
                            mi_fp.ulc_x
                        )
                        l1a_fp.close()
                        print(
                            "Scene %d footprint E=%f S=%f N=%f W=%f"
                            % (
                                scene_id,
                                mi_fp.lrc_x,
                                mi_fp.lrc_y,
                        ...
                        print(
                            "Scene %d footprint too big E=%f S=%f N=%f W=%f"
                            % (
                                scene_id,
                                mi_fp.lrc_x,
                                mi_fp.lrc_y,
        ...
        sf = "Scene_%05d_%s_%s_%s.txt" % (
            orbit,
            sss[0:4] + sss[5:7] + sss[8:13] + sss[14:16] + sss[17:19],
            ses[0:4] + ses[5:7] + ses[8:13] + ses[14:16] + ses[17:19],
            sst[0:4]
            + sst[5:7]
            + sst[8:10]
            + "T"
