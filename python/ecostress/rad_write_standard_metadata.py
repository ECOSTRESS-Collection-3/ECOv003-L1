...
class RadWriteStandardMetadata(WriteStandardMetadata):
    ...
    def __init__(self, *args, line_order_flipped=False,
                 cal_correction = None, **kwargs):
    ...
    def write(self):
        ...
        if(self.cal_correction is not None):
            pg.create_dataset('CalibrationGainCorrection',
                              data=self.cal_correction[0,:],
                              dtype=np.float32)
            pg.create_dataset('CalibrationOffsetCorrection',
