...
class L1aEngSimulate(object):
    ...
    def create_file(self, l1a_eng_fname):
        ...
        data[:, :] = 325
        t = g.create_dataset("RTD_325K", data=np.array(data, dtype=np.float32))
        t.attrs["Units"] = "K and XY"
        data[:, :] = 295
        t = g.create_dataset("RTD_295K", data=np.array(data, dtype=np.float32))
        t.attrs["Units"] = "K and XY"
        t = g.create_dataset("Time_j2000", data=self.time_j2000)
        t.attrs["Units"] = "Seconds"
        m = WriteStandardMetadata(
            fout,
            product_specfic_group="L1A_ENGMetadata",
            pge_name="L1A_RAW_PGE",
            orbit_based=True,
        )
        m.set(
            "RangeBeginningDate",
            self.att_file["/StandardMetadata/RangeBeginningDate"][()],
        )
        m.set(
            "RangeBeginningTime",
            self.att_file["/StandardMetadata/RangeBeginningTime"][()],
        )
