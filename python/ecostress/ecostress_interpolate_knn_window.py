...
# Constants normally imported from ecostress_swig (defined here for testing)
# DQI_INTERPOLATED = 1
...
def find_horizontal_stripes(
        dataset: np.ndarray, data_quality: np.ndarray, n_bands: int, threshold: int = 5) -> np.ndarray:
...
class EcostressAeDeepEnsembleInterpolate(object):
    ...
    def __init__(
        ...
        band_weights: list[float] = [3.0, 0.1, 0.5, 1.0, 3.0],  # Weights for each band in loss function
        verbose: bool = True,
    ) -> None:
        """
    ...
    def create_training_samples(
        self, dataset: np.ndarray, missing_mask: np.ndarray,n_samples: int = 100000  
        ...
        while len(training_x) < n_samples:
            ...
            if np.any(
                missing_mask[grid_slice, col_slice, :] > 0
            ):
                continue
...
class EcostressLocalWindowKNNInterpolator(object):
    ...
    def __init__(
        ...
        self.block_step = block_step if block_step is not None else max(1, window_size // 2)
    ...
    def _select_features_for_center(
        ...
        if self.feature_selection_scope == "center":
            ...
            return selected if len(selected) >= self.min_feature_bands_for_prediction else []
    ...
    def _fit_knn_and_predict(
        ...
        eps = 1e-6
        if batch:
            dists, idxs = model.kneighbors(X_pred, n_neighbors=n_nbrs, return_distance=True)
            ...
            dists, idxs = model.kneighbors(X_pred.reshape(1, -1), n_neighbors=n_nbrs, return_distance=True)
            d = dists[0]
            ii = idxs[0]
            y_nei = y_train[ii].astype(float)
            if y_nei.size == 0:
    ...
    def interpolate_missing(
        self,
        dataset: np.ndarray,
        data_quality: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.exclude_full_bad_edge_columns:
            for band in bands:
                needs_band = (dq_out[:, :, band] != DQI_GOOD) & (dq_out[:, :, band] != DQI_NOT_SEEN)
        ...
        for band in bands:
            needs = (dq_out[:, :, band] != DQI_GOOD) & (dq_out[:, :, band] != DQI_NOT_SEEN)
            ...
            for r0, c0 in block_iter:
                ...
                if self.exclude_full_bad_edge_columns and (touches_left_edge or touches_right_edge):
                    ...
                    for btmp in bands:
                        needs_b_win = (dq_out[rs, cs, btmp] != DQI_GOOD) & (dq_out[rs, cs, btmp] != DQI_NOT_SEEN)
                ...
                def rank_features_by_window_good(fr_candidates: list[int]) -> list[int]:
                    if self.feature_selection_scope == "window_best":
                        ...
                        return [fb_ for fb_, _ in sorted(fr_list, key=lambda t: t[1], reverse=True)]
                    # default: keep original order
                    return list(fr_candidates)

                # Strict: require every center has the feature
                strict_feats = [
                    fb for fb in all_features
                    if all(dq_out[i, j, fb] == DQI_GOOD for (i, j) in centers_abs)
                ]
                strict_feats = rank_features_by_window_good(strict_feats)[: self.max_feature_bands_for_prediction]
                ...
                if len(strict_feats) >= self.min_feature_bands_for_prediction:
                    ...
                    for fb in all_features:
                        good_count = sum(1 for (i, j) in centers_abs if dq_out[i, j, fb] == DQI_GOOD)
                        fr_per_feat.append((fb, good_count / float(num_centers)))
                    ranked_feats = [fb for fb, _ in sorted(fr_per_feat, key=lambda t: t[1], reverse=True)]
                    # Try to pick up to max_feature_bands while ensuring center retention >= 1 - drop_frac
                    for take_k in range(self.max_feature_bands_for_prediction, self.min_feature_bands_for_prediction - 1, -1):
                        ...
                        if keep_count > 0 and (keep_count / float(num_centers)) >= (1.0 - self.center_drop_max_fraction):
                            selected_feats = cand
                            centers_keep_mask = keep
                            break
                    # If still nothing, try any minimal combo that yields at least min centers
                    if len(selected_feats) == 0:
                        for take_k in range(self.min_feature_bands_for_prediction, self.max_feature_bands_for_prediction + 1):
                            cand = ranked_feats[: take_k]
                            if len(cand) < self.min_feature_bands_for_prediction:
                                continue
                            keep = np.array([
                ...
                if len(selected_feats) < self.min_feature_bands_for_prediction or centers_keep_mask is None or not np.any(centers_keep_mask):
                    continue

                # Optionally cap selected features by window quality again
                selected_feats = rank_features_by_window_good(selected_feats)[: self.max_feature_bands_for_prediction]
                ...
                centers_abs_keep = [centers_abs[idx] for idx, k in enumerate(centers_keep_mask) if k]
                if len(centers_abs_keep) == 0:
                    continue
                X_pred = np.stack([
                    np.array([dataset[i, j, fb] for fb in selected_feats], dtype=float)
                    for (i, j) in centers_abs_keep
                ], axis=0)

                # Predict in batch
                y_pred, uq = self._fit_knn_and_predict(X_train, y_train, X_pred, batch=True)
                if y_pred.size == 0:
                    continue
                # Write back
        ...
        dq_out[(dq_out == DQI_STRIPE_NOT_INTERPOLATED) & (dataset > fill_value_threshold)] = DQI_GOOD

        return result, uncertainty, dq_out


__all__.append("EcostressLocalWindowKNNInterpolator")
