...
def test_band_to_band(isolated_dir, igc_hres, lwm, dn_fname, gain_fname):
    ...
    rradsub = geocal.SubRasterImage(
        rrad,
        scan_index * igc_hres.number_line_scan,
        0,
        igc_hres.number_line_scan,
        igc_hres.number_sample,
    )
    geocal.GdalRasterImage.save(
        "b3_before.img", "VICAR", rradsub, geocal.GdalRasterImage.Float64
    )
    refrad = ecostress.EcostressRadApply(
        str(dn_fname),
        str(gain_fname),
        ecostress.EcostressImageGroundConnection.REF_BAND,
    )
    refradsub = geocal.SubRasterImage(
        refrad,
        scan_index * igc_hres.number_line_scan,
        0,
        igc_hres.number_line_scan,
    ...
    geocal.GdalRasterImage.save(
        "b4.img", "VICAR", refradsub, geocal.GdalRasterImage.Float64
    )
    ...
    geocal.GdalRasterImage.save(
        "b3.img", "VICAR", rbreg, geocal.GdalRasterImage.Float64
    )
