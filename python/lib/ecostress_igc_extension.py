...
def _match_overlap(self, matcher, scan, sample):
    ...
    ic1 = geocal.ImageCoordinate(self.number_line_scan * (scan+1) - 1,
                                 sample)
    ic2, success = self.image_coordinate_scan_index(self.ground_coordinate(ic1),
                                                    scan+1)
    if(not success):
    ...
    if(not success):
        return (ic1, ic2, ic2, False, geocal.CcorrMatcher.IMAGE_MASKED)
    ras1 = geocal.SubRasterImage(self.image, self.number_line_scan * scan,
                                 0, self.number_line_scan, self.number_sample)
    ras2 = geocal.SubRasterImage(self.image, self.number_line_scan * (scan+1),
                                 0, self.number_line_scan, self.number_sample)
...
def _match_all_overlap(self, min_corr = 0.90, min_var = 50.0,
                       sample_step = 100):
    ...
    m = geocal.CcorrLsmMatcher(geocal.CcorrMatcher(15, 15, 9, 9,min_corr,min_var),
                               geocal.LsmMatcher(11, 11))
    res = []
