...
def test_gps_time_normal():
    ...
    out = calc.gps_time(time_fsw, sync_fsw, sync_fpie)
    np.testing.assert_allclose(out, real_times - 0.05, atol=1e-6)


def test_gps_time_corrupt_bad_time_error_correction_burst():
...
def test_gps_time_corrupt_time_fsw():
...
def test_gps_time_falls_back_when_window_fully_corrupted():
