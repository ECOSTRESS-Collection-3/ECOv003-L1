...
def load_data(data_path, size=None):
    ...
    # Auto-detect file format and set appropriate paths
    if "/Radiance/radiance_1" in f:
        # Old format (L1B_RAD files)
        radiance_path = "/Radiance/radiance_%d"
        data_quality_path = "/Radiance/data_quality_%d"
        print("Detected old format: using /Radiance/ paths")
        ...
        print("Detected new format: using /HDFEOS/GRIDS/ paths")
    else:
    ...
    # Create dataset array using the first radiance band to get shape
    first_radiance_path = radiance_path % 1
    dataset = np.zeros((*f[first_radiance_path].shape, 5))
    data_quality = np.zeros(dataset.shape)

    # Load all bands and their quality indicators
    for b in range(5):
        dataset[:, :, b] = f[radiance_path % (b + 1)]
        data_quality[:, :, b] = f[data_quality_path % (b + 1)]

    # Handle NaN values in raw data by converting them to appropriate fill values
    ...
    if np.any(nan_mask):
        ...
        for b in range(5):
            ...
            if np.any(remaining_nan):
                dataset[remaining_nan, b] = FILL_VALUE_BAD_OR_MISSING
...
def scatter_plot_true_vs_predicted(
    ...
    for band in bands_to_process:
        ...
        # make sure x and y lim are the same if we have data
        if len(true_band) >= 2:
            lim = np.nanpercentile(true_band, [1, 99])
            if np.all(np.isfinite(lim)) and lim[0] != lim[1]:
                axs[i].set_xlim(lim)
                axs[i].set_ylim(lim)
        axs[i].legend()
...
def save_interpolated_file(
    ...
    with h5py.File(output_file_path, "r+") as f:
        ...
        if "/Radiance/radiance_1" in f:
            ...
            radiance_path = "/Radiance/radiance_%d"
            data_quality_path = "/Radiance/data_quality_%d"
            uncertainty_path = "/Radiance/interpolation_uncertainty_%d"
            print("Detected old format: updating /Radiance/ paths")
        elif "/HDFEOS/GRIDS/ECO_L1CG_RAD_70m/Data Fields/radiance_1" in f:
            # New format (L1CG files)
            radiance_path = "/HDFEOS/GRIDS/ECO_L1CG_RAD_70m/Data Fields/radiance_%d"
            data_quality_path = (
        ...
        for b in range(n_bands):
            ...
            if rad_path in f:
                ...
                chunks = orig_rad_dataset.chunks
            ...
            if interpolation_uncertainty is not None:
                ...
                # Remove if it exists (it probably doesn't in the original)
                if unc_path in f:
                    del f[unc_path]
...
def benchmark_interpolate(
    ...
    interpolator = EcostressLocalWindowKNNInterpolator(
        ...
        n_neighbors=local_knn_neighbors,
...
for file_path in files:
    ...
    scene_elapsed = int(time.time() - scene_start_time)
    m, s = divmod(scene_elapsed, 60)
    print(f"Scene {scene_name_timer} runtime: {m}m {s}s")
...
m_total, s_total = divmod(total_elapsed, 60)
print(f"Total runtime: {m_total}m {s_total}s")
print("Done >>>")
