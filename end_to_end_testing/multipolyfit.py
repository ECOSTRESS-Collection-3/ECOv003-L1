# This is from the pip package multipolyfit, currently hosted at
# https://github.com/mrocklin/multipolyfit.
#
# We could just install the packages, but there are several changes we needed
# to make (this hasn't been updated in 4 years). Since this is small, we
# just copy the code in place here and use.
#
...
def multipolyfit(xs, y, deg, full=False, model_out=False, powers_out=False):
    ...
    beta, residuals, rank, s = linalg.lstsq(A, y)
    ...
    if powers_out:
        return beta, powers
    if full:
        return beta, residuals, rank, s, powers
    return beta
